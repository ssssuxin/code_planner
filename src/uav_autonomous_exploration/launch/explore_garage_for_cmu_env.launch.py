...
def generate_launch_description():
    ...
    uav_planner_node = Node(
        ...
        parameters=[             
                    ...
                    {"kExtendWayPointDistanceBig":0.5},
                    ...
                    {"valid_frontier_point_num_thred":50},#100以内的簇才有资格让frontier viewpoint来操作
                    ...
                    {"intervel_radius":1.0}, #frontier点之间不小于
                    {"para_visited_radius":4.5},#标记访问了半径
                    {"least_visible_unknown_num":200},#覆盖到未标绿点小于这个数 则使命结束
        ...
        # 重映射 position_cmd 话题
            ...
            ('/uav_state_estimation', '/state_estimation'),
            ('/terrain_map_ext', "/terrain_map_ext"),
            ('/terrain_map', "/terrain_map"),
            ('/Pub_switch', "/Pub_switch"),
            ('/way_point', '/move_base_simple/goal'),#point类型，用于给ros2的ego发消息
    ...
    rviz_para_dir = os.path.join(get_package_share_directory('uav_planner'), 'rviz', 'uav_planner_garage_for_cmu_env.rviz')
    rviz_node = Node(
        package="rviz2",
        executable="rviz2",
        name="rviz2",
