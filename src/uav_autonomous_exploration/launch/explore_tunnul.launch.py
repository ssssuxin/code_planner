...
#M? 可能出现的问题  connected的整体错位：   这是由于机器人位姿一直是靠墙导致 在  局部框滚动 的时候一直沿用的上一次的ind_of_connected_viewpoint 出现一帧基本上就完蛋了（会影响地铁构建）  所以每个小中方格适当选大一点
    uav_planner_node = Node(
        ...
        parameters=[
                    ...
                    {"kExtendWayPointDistanceBig":3.0},
                    {"kExtendWayPointDistanceSmall":2.5},
                    ...
                    {"valid_frontier_point_num_thred":200},#100以内的簇才有资格让frontier viewpoint来操作
                    ...
                    {"kPointCloudCellSize":9.0},#这个是一个格子occupancy的长  下面是宽
                    ...
                    {"para_visited_radius":2.5},#标记访问了半径
                    ...
                    {"search_radius_thr":0.1},#如果frontier viewpoint点卡在墙里了，用周围 1 内的空闲点设置新的position
                    ...
                    {"kextendz":False},
                    {"pub_mode_":2}, #0是原版 1是发一次环境再更新一次 其余数值为路径规划一直工作，不等待环境更新
        ...
        # 重映射 position_cmd 话题
            ...
            ('/way_point', '/move_base_simple/goal'),#pose类型 用于给ros1的那个环境发消息
    ...
    rviz_para_dir = os.path.join(get_package_share_directory('uav_planner'), 'rviz', 'uav_planner_tunnel.rviz')
    rviz_node = Node(
        package="rviz2",
        executable="rviz2",
        name="rviz2",
