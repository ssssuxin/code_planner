...
def generate_launch_description():
    ...
    uav_planner_node = Node(
        ...
        parameters=[             
                    ...
                    {"kExtendWayPointDistanceSmall":1.2},
                    ...
                    {"keypose_graph/ckeck_loger_than_length":2},  #去除过 错误加入的kepose 跨越未知区域路径  的时候使用  ①选
                    {"keypose_graph/eliminate_edge_non_free_thr":5},#出现10个nonfree以上的我们认为是错误跨越的路径  给它去除掉②除掉  
                    # # ViewPointManager
                    ...
                    {"viewpoint_manager/resolution_z":1.2},
                    {"viewpoint_manager/big_box_size_x":5},
                    {"viewpoint_manager/big_box_size_y":5},
                    {"viewpoint_manager/big_box_size_z":3},
                    {"viewpoint_manager/min_box_size_x":8},
                    {"viewpoint_manager/min_box_size_y":8},
                    {"viewpoint_manager/min_box_size_z":3},
                    {"viewpoint_manager/incollision_barrier_level":1},#0是自己裸奔， 1是四周往外一层occupy  2是两层 3是三层
                    {"viewpoint_manager/first_frame_grace":True},
                    {"check_collision_margin":10},#这里是把1/5的边搞掉不判断
                    {"intervel_radius":10.0}, #frontier点之间不小于
                    {"para_visited_radius":13.5},#标记访问了半径
                    {"least_visible_unknown_num":100},#覆盖到未标绿点小于这个数 则使命结束
                    {"least_visible_free_num":100},
                    {"Set_robot_neigbour_connected":False},
                    {"in_collision_viewpoint_get_connected_viewpoint_dis":1.2},
                    {"search_radius_thr":1.0},#如果frontier viewpoint点卡在墙里了，用周围 1 内的空闲点设置新的position
                    ...
                    {"kMinAddPointNumSmall":10},#候选点包含 墙面点 的最低数量（local 和 grid world）
                    {"kMinAddPointNumBig":60},  #gridworld中为了返exploring 候选点包含 墙面点 的数量
                    {"kMinAddFrontierPointNum":5},#候选点包含 frontier 的最低数量（local 和 grid world）
                    {"kCellExploringToCoveredThr":1}, #候选点小于这个数量 设定covered
                    {"kCellCoveredToExploringThr":10}, #候选个数 大于这个数量 才能反exploring
