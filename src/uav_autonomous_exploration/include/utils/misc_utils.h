...
template <typename T>
T getParam(std::shared_ptr<rclcpp::Node>& nh, const std::string& name, const T default_val)
{
  //EXPL 已经清理
  T val;
  bool success = nh->get_parameter(name,val);
  if (!success)
  {
    
    RCLCPP_ERROR(nh->get_logger(), "Cannot read parameter: %s", name.c_str());
    return default_val;
  }
  return val;
}
template <typename T>
T getParam(rclcpp::Node& nh, const std::string& name, const T default_val)
{
  T val;
  bool success = nh.get_parameter(name,val);
  if (!success)
...
void PublishCloud(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr& cloud_publisher, const PCLPointCloudType& cloud, const std::string& frame_id)
{
...
void Publish(const typename rclcpp::Publisher<ROSMsgType>::SharedPtr& publisher, ROSMsgType& msg, const std::string& frame_id)
{
...
public:
  ...
  explicit Marker(std::shared_ptr<rclcpp::Node>& nh, std::string pub_topic, std::string frame_id)
  ...
  explicit Marker(rclcpp::Node& nh, std::string pub_topic, std::string frame_id)
    : pub_topic_(pub_topic), frame_id_(frame_id)
  {
    marker_pub_ = nh.create_publisher<visualization_msgs::msg::Marker>(pub_topic_, 2);
    id_++;
