...
{
public:
  PlanningEnv(std::shared_ptr<rclcpp::Node>& nh,std::shared_ptr<rclcpp::Node>& nh_p, std::string world_frame_id = "map");
  ~PlanningEnv() = default;
  ...
  void UpdateRobotPosition(geometry_msgs::msg::Point robot_position)
  {
    ...
    *(KNOWN_WORLD->cloud_) = *(points_from_occupancy_cloud_grid_->cloud_)+*(rolling_occupancy_grid_->GetRolledOut_gather_points_from_occupancy_array_());
    KNOWN_WORLD->Publish();//可视化  这个是已知空间，也就是 “free”+“障碍” 的空间
    ...
    if (occupancy_grid_rolling)
    {
      ...
      pointcloud_manager_->GetOccupancyCloud(points_from_occupancy_cloud_grid_->cloud_);
      // *(points_from_occupancy_cloud_grid_->cloud_)=
      // points_from_occupancy_cloud_grid_->Publish();
    }
  ...
  void UpdateRegisteredCloud(typename pcl::PointCloud<PCLPointType>::Ptr& cloud)
  {
    if (cloud->points.empty())
    {
      ...
      RCLCPP_WARN(rclcpp::get_logger("my_logger"), "PlanningEnv::UpdateRegisteredCloud(): registered cloud empty");
      return;
    }
    else
    {
  ...
  void seperate_point_from_occupancy_array_()
  {
    if(rolling_occupancy_grid_->return_initialized_())//这几行代码应该在每5轮上交的时候运行，另外也可以用一行数组专门维护有哪些ind值值得看，而不是每次都遍历
    {
      // std::cout<<"111"<<std::endl;
      rolling_occupancy_grid_->Get_free_occupy_pointcloud_fromoccupancy_array_();
      // std::cout<<"1222"<<std::endl;
      rolling_occupancy_grid_->Return_free_occupy_pointcloud(*Local_free_points_->cloud_,*Local_Occupy_points_->cloud_,*Occupancy_non_free_cloud->cloud_,*Occupancy_unknow_cloud->cloud_);
      // std::cout<<"333"<<std::endl;
      if(!Occupancy_non_free_cloud->cloud_->empty())
      {
        
        local_pcl_map_for_ego_flag = true;
        
        local_pcl_map_for_ego_cloud_kdtree_->setInputCloud(Occupancy_non_free_cloud->cloud_);
        
        // std::cout<<"----------"<<std::endl;
      }
      Occupancy_non_free_cloud->Publish();//可视化
      Local_free_points_->Publish();//可视化
      Local_Occupy_points_->Publish();//可视化
    }
  }
  ...
  void UpdateKeyposeCloud(typename pcl::PointCloud<PCLPointType>::Ptr& keypose_cloud)
  {
    ...
    {
      // ROS_WARN(
      RCLCPP_WARN(rclcpp::get_logger("my_logger"), "PlanningEnv::UpdateKeyposeCloud(): keypose cloud empty");
      return;
    }
    else
    {
      ...
      *vertical_surface_cloud_stack_[keypose_cloud_count_] = *(vertical_surface_cloud_->cloud_);
      ...
      // {
      //   local_pcl_map_for_ego_cloud_kdtree_->setInputCloud(stacked_cloud_->cloud_);
      // }
  ...
  void UpdateCoveredArea_uav(const lidar_model_ns::LiDARModel& robot_viewpoint,
  ...
  void GetUncoveredArea_uav(const std::shared_ptr<uav_viewpoint_manager_ns::ViewPointManager>& uav_viewpoint_manager,
                                   int& uncovered_point_num, int& uncovered_frontier_point_num);
  ...
  void Setpointcloud_manager_invalidate_viewpoint_and_point_in_covered_boxes(bool flag)
  {
    pointcloud_manager_->Set_invalidate_viewpoint_and_point_in_covered_boxes(flag);
  }
  std::unique_ptr<rolling_occupancy_grid_ns::RollingOccupancyGrid> & Get_rolling_occupancy_grid_()
  {
    return rolling_occupancy_grid_;
  }
  std::shared_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>> frontier_center_point;
  std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>> Occupancy_unknow_cloud;//需要使用
  std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>> Occupancy_unknow_cloud_for_frontier_viewpoint;//需要使用
  std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>> Local_free_points_;//需要使用
  std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>> Local_free_points_for_frontier_viewpoint;//需要使用
  std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>> Cantidate_frontier_viewpoint;
  std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>> In_field_frontier_viewpoint;
  std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>> All_frontier_viewpoint;
private:
  bool local_pcl_map_for_ego_flag;
  ...
  // std::unique_ptr<pointcloud_utils_ns::PCLCloud<PlannerCloudPointType>> local_pcl_map_for_ego_inputkd_cloud;
  pcl::KdTreeFLANN<PlannerCloudPointType>::Ptr stacked_vertical_surface_cloud_kdtree_;
  pcl::KdTreeFLANN<pcl::PointXYZI>::Ptr local_pcl_map_for_ego_cloud_kdtree_;
  ...
  std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>> points_from_occupancy_cloud_grid_;
  std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>> KNOWN_WORLD;//可视化
  
  std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>> Local_Occupy_points_;//可视化
  std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>> Occupancy_non_free_cloud;//需要使用
  
  std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZINormal>> Local_normal_points;//可视化
  ...
  std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>> uav_uncovered_cloud_;
  std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>> uncovered_frontier_cloud_;
  std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>> uav_uncovered_frontier_cloud_;
  ...
  std::vector<std::shared_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>> frontier_point_clouds_vec;
  int valid_length_of_frontier_point_clouds_vec;
  int valid_frontier_point_num_thred;
