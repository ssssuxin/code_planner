...
{
public:
  explicit LocalCoveragePlanner(std::shared_ptr<rclcpp::Node>& nh);
  ...
  void SetUavViewPointManager(std::shared_ptr<uav_viewpoint_manager_ns::ViewPointManager> const& uav_viewpoint_manager)
  {

    uav_viewpoint_manager_ = uav_viewpoint_manager;
    use_frontier_ = uav_viewpoint_manager_->UseFrontier();
...
private:
  ...
  void SelectViewPoint(const std::vector<std::pair<int, int>>& queue, const std::vector<bool>& covered,
                       std::vector<int>& selected_viewpoint_Hier_indices, bool use_frontier = false);
  ...
  exploration_path_ns::ExplorationPath SolveTSP(const std::vector<int>& selected_viewpoint_Hier_indices,
  ...
  std::vector<int> last_selected_viewpoint_Hier_indices_;
};
}  // namespace local_coverage_planner_ns