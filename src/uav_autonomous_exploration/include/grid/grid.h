...
public:
  ...
  // explicit Grid(int A,const Eigen::Vector3i& size, _U init_value, const Eigen::Vector3d& origin = Eigen::Vector3d(0, 0, 0),
  //               const Eigen::Vector3d& resolution = Eigen::Vector3d(1, 1, 1), int dimension = 3)
  // // (,int A=0)
  // {
  //   // MY_ASSERT(size.x() > 0);
  ...
  //   std::shared_ptr<grid_ns::Grid<_U,int>> mid_box_temp;//跟push_back配合，把指针复制后压进cells里
  //   for (int i = 0; i < cell_number_; i++)
  //   {
  //     cells_.push_back(mid_box_temp);
  //     subs_.push_back(ind2sub_(i));
  //   }
  // }
  ...
  Eigen::Vector3i Pos2Sub_for_outside_point(const Eigen::Vector3d& pos) const
  {
    Eigen::Vector3i sub(0, 0, 0);
    for (int i = 0; i < dimension_; i++)
    {
      sub(i) =  static_cast<int>((pos(i) - origin_(i)) * resolution_inv_(i)) ;
    }
    return sub;
  }
