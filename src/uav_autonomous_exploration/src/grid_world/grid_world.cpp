...
GridWorld::GridWorld(std::shared_ptr<rclcpp::Node>& nh) : initialized_(false), use_keypose_graph_(false)
{
  ReadParameters(nh);
  ...
  global_viewpoint_cloud = std::make_unique<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>(
      nh, "global_viewpoint_cloud", "map");//可视化
}
...
void GridWorld::ReadParameters(std::shared_ptr<rclcpp::Node>& nh)
{
  ...
  // double viewpoint_number = misc_utils_ns::getParam<double>(nh, "viewpoint_manager/number_x", 40);
  ...
  double min_box_size_x = misc_utils_ns::getParam<int>(nh, "viewpoint_manager/min_box_size_x", 10);
  double min_box_size_z = misc_utils_ns::getParam<int>(nh, "viewpoint_manager/min_box_size_z", 4);
  resolution_x   = misc_utils_ns::getParam<double>(nh, "viewpoint_manager/resolution_x", 0.6);
  double resolution_z   = misc_utils_ns::getParam<double>(nh, "viewpoint_manager/resolution_z", 0.6);
  kCellSize =   min_box_size_x * resolution_x ;
  kCellHeight =   min_box_size_z * resolution_z ;
  longer_side_length = big_box_size_x*min_box_size_x * resolution_x ;
                    ...
                    // {"viewpoint_manager/min_box_size_z":4},
  ...
  Estime_any_Frontier_viewpoint_asexploring = misc_utils_ns::getParam<bool>(nh, "Estime_any_Frontier_viewpoint_asexploring", false);
}
...
void GridWorld::UpdateNeighborCells(const geometry_msgs::msg::Point& robot_position)
{
  ...
  // GetNeighborCellIndices(robot_position, Eigen::Vector3i(1, 1, 1), neighbor_cell_indices_3_3_3_);
...
void GridWorld::Addcantidate_frontier_ViewPointToCell(int cell_ind, int viewpoint_ind)
{
  MY_ASSERT(subspaces_->InRange(cell_ind));
  subspaces_->GetCell(cell_ind).Addcantidate_frontier_viewpoint_indices_(viewpoint_ind);
}
void GridWorld::Addboundary_ViewPointToCell(int cell_ind, int viewpoint_ind)
{
  MY_ASSERT(subspaces_->InRange(cell_ind));
  subspaces_->GetCell(cell_ind).Addboundary_viewpoint_indices_(viewpoint_ind);
}
void GridWorld::Setfrontier_ViewPoint_sizeToCell(int cell_ind)
{
  MY_ASSERT(subspaces_->InRange(cell_ind));
  subspaces_->GetCell(cell_ind).Add_frontier_viewpoint_size_indices_();
}
...
void GridWorld::UpdateCellStatus(const std::unique_ptr<planning_env_ns::PlanningEnv>& planning_env, const std::unique_ptr<keypose_graph_ns::KeyposeGraph>& keypose_graph, const std::shared_ptr<uav_viewpoint_manager_ns::ViewPointManager>& uav_viewpoint_manager, std::vector<int>& global_cell_covered_marker_vec)
{
  int exploring_count = 0;
  int unseen_count = 0;
  int covered_count = 0;
  ...
  // std::cout<<"EXPLORING: "<<exploring_count<<std::endl;
  // std::cout<<"UNSEEN: "<<unseen_count<<std::endl;
  // std::cout<<"COVERED: "<<covered_count<<std::endl;

  for (const auto& cell_ind : neighbor_cell_indices_)
  {
    subspaces_->GetCell(cell_ind).ClearViewPointIndices();
    subspaces_->GetCell(cell_ind).Clearcantidate_frontier_viewpoint_indices_();
    subspaces_->GetCell(cell_ind).Clearfrontier_viewpoint_size();
    subspaces_->GetCell(cell_ind).Clearboundary_viewpoint_indices_();
  }
  for(int i=0;i<uav_viewpoint_manager->ind_of_connected_viewpoints.size();i++)//每个元素
  {
    geometry_msgs::msg::Point viewpoint_position = uav_viewpoint_manager->Get_littlebox_from_Hierarchy_ind(uav_viewpoint_manager->ind_of_connected_viewpoints[i])->GetPosition(); 
    Eigen::Vector3i sub =
    ...
    if (subspaces_->InRange(sub))
    {
      ...
      AddViewPointToCell(cell_ind, uav_viewpoint_manager->ind_of_connected_viewpoints[i]);
      uav_viewpoint_manager->SetViewPointCellInd(uav_viewpoint_manager->ind_of_connected_viewpoints[i], cell_ind);
      // viewpoint_manager->SetViewPointCellInd(viewpoint_ind, cell_ind);
    }
    else
    {
      // ROS_ERROR_STREAM("subspace sub out of bound: " << sub.transpose());
      RCLCPP_ERROR_STREAM(rclcpp::get_logger("my_logger"), "subspace sub out of bound: " << sub.transpose());

    }
  }
  for(auto Hier_ind : uav_viewpoint_manager->Hier_ind_of_cantidate_frontier_viewpoints)
  {
    geometry_msgs::msg::Point frontier_viewpoint_position = uav_viewpoint_manager->Get_Frontier_viewpoint_from_Hier_ind(Hier_ind)->GetPosition();
    Eigen::Vector3i sub =
        subspaces_->Pos2Sub(Eigen::Vector3d(frontier_viewpoint_position.x, frontier_viewpoint_position.y, frontier_viewpoint_position.z));
    if (subspaces_->InRange(sub))
    {
      int cell_ind = subspaces_->Sub2Ind(sub);
      Addcantidate_frontier_ViewPointToCell(cell_ind, Hier_ind);
      // uav_viewpoint_manager->SetViewPointCellInd(uav_viewpoint_manager->ind_of_connected_viewpoints[i], cell_ind);
      // viewpoint_manager->SetViewPointCellInd(viewpoint_ind, cell_ind);
    }
    else
  ...
  // if(uav_viewpoint_manager->boundary_index.size()>0)
  for(auto Hier_ind : uav_viewpoint_manager->boundary_index)
  {
    geometry_msgs::msg::Point frontier_viewpoint_position = uav_viewpoint_manager->Get_littlebox_from_Hierarchy_ind(Hier_ind)->GetPosition();
    Eigen::Vector3i sub =
        subspaces_->Pos2Sub(Eigen::Vector3d(frontier_viewpoint_position.x, frontier_viewpoint_position.y, frontier_viewpoint_position.z));
    if (subspaces_->InRange(sub))
    {
      int cell_ind = subspaces_->Sub2Ind(sub);
      Addboundary_ViewPointToCell(cell_ind, Hier_ind);
      // uav_viewpoint_manager->SetViewPointCellInd(uav_viewpoint_manager->ind_of_connected_viewpoints[i], cell_ind);
      // viewpoint_manager->SetViewPointCellInd(viewpoint_ind, cell_ind);
    }
    else
  ...
  for(auto frontier_viewpoint : uav_viewpoint_manager->Return_frontier_viewpoints())
  {
    //TODO
    if(!frontier_viewpoint->IsCandidate())//这里是因为穿墙问题，所以说只计算那些能局部路径规划的，后面如果解决了穿墙问题可以去掉这段，保留frontier viewpoint对全局路径规划的影响
    {
      continue;
    }
    geometry_msgs::msg::Point frontier_viewpoint_position = frontier_viewpoint->GetPosition();
    Eigen::Vector3i sub =
        subspaces_->Pos2Sub(Eigen::Vector3d(frontier_viewpoint_position.x, frontier_viewpoint_position.y, frontier_viewpoint_position.z));
    if (subspaces_->InRange(sub))
    {
      int cell_ind = subspaces_->Sub2Ind(sub);
      Setfrontier_ViewPoint_sizeToCell(cell_ind);
      // uav_viewpoint_manager->SetViewPointCellInd(uav_viewpoint_manager->ind_of_connected_viewpoints[i], cell_ind);
      // viewpoint_manager->SetViewPointCellInd(viewpoint_ind, cell_ind);
    }
    else
  ...
  //下面这段是调整visitd半径的，如果机器人所在cell里面connected视点太少，那么证明处于狭小空间，减少访问半径
  //这段的作用现在在UpdateViewPointVisited重现了
  // Eigen::Vector3i sub =
  //     subspaces_->Pos2Sub(Eigen::Vector3d(robot_position_.x, robot_position_.y, robot_position_.z));
  // if (subspaces_->InRange(sub))
  // {
  //   int cell_ind = subspaces_->Sub2Ind(sub);
  //   if(subspaces_->GetCell(cell_ind).GetViewPointIndices().size() < uav_viewpoint_manager->Get_middle_box_viewpoint_num()/3)
  //   {
  //     uav_viewpoint_manager->Retrun_visited_rad() = uav_viewpoint_manager->Retrun_visited_rad()/3;
  //     // std::cout<<"访问半径减小"<<std::endl;
  //   }
  //   else{
  //     uav_viewpoint_manager->Retrun_visited_rad() = uav_viewpoint_manager->Retrun_visited_rad_copy();
  //   }
  // }

  Eigen::Vector3d temp_3d;
  temp_3d.x() = robot_position_.x;
  temp_3d.y() = robot_position_.y;
  temp_3d.z() = robot_position_.z;
  int rb_cell_ind = subspaces_->Pos2Ind(temp_3d);
  temp_3d = subspaces_->Ind2Pos(rb_cell_ind);
  geometry_msgs::msg::Point rb_cell_center_point;
  rb_cell_center_point.x = temp_3d.x();
  rb_cell_center_point.y = temp_3d.y();
  rb_cell_center_point.z = temp_3d.z();
  for (const auto& cell_ind : neighbor_cell_indices_)
  {
    ...
    int highest_score = -1;
    int A_selected_viewpoint_Hier_ind;
    for (const auto& uav_viewpoint_Hier_ind : subspaces_->GetCell(cell_ind).GetViewPointIndices())
    {
      MY_ASSERT(uav_viewpoint_manager->Get_littlebox_from_Hierarchy_ind(uav_viewpoint_Hier_ind)->Connected());
      candidate_count++;
      if(uav_viewpoint_manager->Get_littlebox_from_Hierarchy_ind(uav_viewpoint_Hier_ind)->Selected())
      {
        A_selected_viewpoint_Hier_ind = uav_viewpoint_Hier_ind;
        selected_viewpoint_count++;
      }
      if (uav_viewpoint_manager->Get_littlebox_from_Hierarchy_ind(uav_viewpoint_Hier_ind)->Visited())
      {
        continue;
      }
      int score = uav_viewpoint_manager->Get_littlebox_from_Hierarchy_ind(uav_viewpoint_Hier_ind)->GetCoveredPointNum();
      int frontier_score = uav_viewpoint_manager->Get_littlebox_from_Hierarchy_ind(uav_viewpoint_Hier_ind)->GetCoveredFrontierPointNum();
      if (score > highest_score)
      {
        highest_score = score;
        highest_score_viewpoint_ind = uav_viewpoint_Hier_ind;
      }
    ...
    if(uav_viewpoint_manager->use_frontier_viewpoint)
    {
      if(Estime_any_Frontier_viewpoint_asexploring)
      {
        selected_viewpoint_count += subspaces_->GetCell(cell_ind).Getfrontier_viewpoint_size_();
        candidate_count += subspaces_->GetCell(cell_ind).Getfrontier_viewpoint_size_();
      }
      else{
        selected_viewpoint_count += subspaces_->GetCell(cell_ind).Getcantidate_frontier_viewpoint_indices_().size();
        candidate_count += subspaces_->GetCell(cell_ind).Getcantidate_frontier_viewpoint_indices_().size();
      }
        ...
        candidate_count > 0)
    {
      ...
      // double path_length = keypose_graph->GetShortestPath(subspaces_->GetCell(cell_ind).Getconnection_point_geo(),
      //                                                     // rb_cell_pos, false, path_tmp, false);
      //                                                     robot_position_, false, path_tmp, false);
      // if(path_length>0.1&&path_length<longer_side_length*1.74)//
      subspaces_->GetCell(cell_ind).SetStatus(CellStatus::COVERED);
      // std::cout<<"大哥setcovered"<<std::endl;
    ...
    // Covered to Exploring
             ...
             (above_big_threshold_count >= kCellCoveredToExploringThr ||
              ...
              subspaces_->GetCell(cell_ind).Getcantidate_frontier_viewpoint_indices_().size()>0 ) )
    ...
    //EXPL 
             ...
             candidate_count > 10 && subspaces_->GetCell(cell_ind).Getcantidate_frontier_viewpoint_indices_().size()==0)
    {//这个地方只是为了实现探索效率，提前让信息量少的地方结束探索
      auto cell_sub = subspaces_->Ind2Sub(cell_ind);
      int neibour_exploring_count = 0;
      for(int x_ = -1; x_ < 1; x_++)
      {
        for(int y_ = -1; y_ < 1; y_++)
        {
          ...
          {
            Eigen::Vector3i neibour_sub;
            neibour_sub.x() = cell_sub.x() + x_;
            neibour_sub.y() = cell_sub.y() + y_;
            neibour_sub.z() = cell_sub.z() + z_;
            if(subspaces_->InRange(neibour_sub))
            {
              auto neibour_ind = subspaces_->Sub2Ind(neibour_sub);
              if(subspaces_->GetCell(neibour_ind).GetStatus() == CellStatus::EXPLORING)
              neibour_exploring_count++;
            }
            
          }
        }
      }
      if(neibour_exploring_count > 1)
      {
        subspaces_->GetCell(cell_ind).SetStatus(CellStatus::COVERED);
      }
    ...
    //EXPL ①这里是入局的点 UNSEEN出状态直接变成 EXPLORING  （没别的功能）
    else if (subspaces_->GetCell(cell_ind).GetStatus() != CellStatus::COVERED && selected_viewpoint_count > subspaces_->GetCell(cell_ind).Getboundary_viewpoint_indices_().size())
    {
      subspaces_->GetCell(cell_ind).SetStatus(CellStatus::EXPLORING);
    ...
    else if (subspaces_->GetCell(cell_ind).GetStatus() == CellStatus::EXPLORING && candidate_count == 0)
    {
      ...
      {
        subspaces_->GetCell(cell_ind).SetStatus(CellStatus::COVERED);
        std::cout<<"first visited and set covered"<<std::endl;
      }
      else
      {
        ...
        double z_dist_to_robot = std::abs(cell_position.z - robot_position_.z);
          ...
          if (xy_dist_to_robot < kCellSize && z_dist_to_robot < kCellHeight )
          {

            
            if(subspaces_->GetCell(cell_ind).Is_reachable())
            {
              geometry_msgs::msg::Point rb_cell_pos = subspaces_->GetCell(rb_cell_ind).Getconnection_point_geo();
              if(!subspaces_->GetCell(rb_cell_ind).Is_reachable())
              {
                rb_cell_pos = subspaces_->GetCell(rb_cell_ind).GetPosition();
              }

              double temp_dist =
                  // misc_utils_ns::PointXYZDist<geometry_msgs::msg::Point, geometry_msgs::msg::Point>(cell_position, rb_cell_center_point);
                  misc_utils_ns::PointXYZDist<geometry_msgs::msg::Point, geometry_msgs::msg::Point>(subspaces_->GetCell(cell_ind).Getconnection_point_geo(), global_path_robot_position);
              nav_msgs::msg::Path path_tmp;

              
              double path_length = keypose_graph->GetShortestPath(subspaces_->GetCell(cell_ind).Getconnection_point_geo(),
                                                                  // rb_cell_pos, false, path_tmp, false);
                                                                  global_path_robot_position, false, path_tmp, false);
              
              if(path_length >0.1 && 
              // (temp_dist>path_length || 
              // abs(temp_dist-path_length) < 1.0  || 
              path_length<uav_viewpoint_manager->Return_big_box_bigger_side() )
              {
                subspaces_->GetCell(cell_ind).SetStatus(CellStatus::COVERED);
                std::cout<<temp_dist<<"   ||  "<<keypose_graph->GetShortestPath(subspaces_->GetCell(cell_ind).Getconnection_point_geo(),
                                                                  global_path_robot_position, false, path_tmp, false)<<std::endl;
                std::cout<<"使用了grid_world的放错机制-------------------"<<std::endl;
              }
            }
            
            // if(keypose_graph->Find_edge_connected(subspaces_->GetCell(cell_ind).GetGraphNodeIndices(),subspaces_->GetCell(rb_cell_ind).GetGraphNodeIndices()))
            // {
            //   subspaces_->GetCell(cell_ind).SetStatus(CellStatus::COVERED);
            //   std::cout<<"使用了grid_world的放错机制"<<std::endl;
            // }
            Eigen::Vector3d temp_3d,temp_3d2;
            temp_3d.x() = global_path_robot_position.x;
            temp_3d.y() = global_path_robot_position.y;
            temp_3d.z() = global_path_robot_position.z;
            auto point_ = subspaces_->GetCell(cell_ind).Getconnection_point_geo();
            temp_3d2.x() = point_.x;
            temp_3d2.y() = point_.y;
            temp_3d2.z() = point_.z;
            if(planning_env->Get_rolling_occupancy_grid_()->Visible_in_RayTraceHelper_for_viewpoint_connected(temp_3d2,temp_3d,false,0))
            {
              subspaces_->GetCell(cell_ind).SetStatus(CellStatus::COVERED);
              std::cout<<"防错机制2"<<std::endl;
            }
          }
      ...
      //TODO if()//之间只有一点keypose连接  工作
      //TODO 这个地方就是吃了参数的亏  看下别的是不是也有参数限制的问题
    }
    ...
    if(subspaces_->GetCell(cell_ind).GetStatus() == CellStatus::COVERED)
    {
      global_cell_covered_marker_vec_.push_back(cell_ind);
    }

    // if(subspaces_->GetCell(cell_ind).GetStatus() == CellStatus::EXPLORING && !subspaces_->GetCell(cell_ind).Is_reachable())
    // {//放错机制，针对那些地铁还没搭好但是机器人转头离开导致的  无法relocation
    //   auto cell_sub = subspaces_->Ind2Sub(cell_ind);
    //   int neibour_exploring_count = 0;
    //   for(int x_ = -1; x_ < 1; x_++)
    //   {
    ...
    //         {
    //           auto neibour_ind = subspaces_->Sub2Ind(neibour_sub);
    //           if(subspaces_->GetCell(neibour_ind).Is_reachable())
    //           {
    //             subspaces_->GetCell(cell_ind).Setconnection_point_geo(subspaces_->GetCell(neibour_ind).Getconnection_point_geo());
    //             subspaces_->GetCell(cell_ind).Setreachable(true);
    //             break;
    //           }
    //         }
    ...
    // if(selected_viewpoint_count>0 && !subspaces_->GetCell(cell_ind).Is_reachable())
    // {//放错机制，针对那些地铁还没搭好但是机器人转头离开导致的  无法relocation
    //   bool do_flag=true;
    //   auto cell_sub = subspaces_->Ind2Sub(cell_ind);
    //   for(int x_ = -1; x_ < 1; x_++)
    ...
    //         {
    //           auto neibour_ind = subspaces_->Sub2Ind(neibour_sub);
    //           if(cell_ind == neibour_ind)
    //             continue;
    //           if(subspaces_->GetCell(neibour_ind).Is_reachable())
    //           {
    //             // subspaces_->GetCell(cell_ind).Setconnection_point_geo(subspaces_->GetCell(neibour_ind).Getconnection_point_geo());
    //             // subspaces_->GetCell(cell_ind).Setreachable(true);
    //             // subspaces_->GetCell(cell_ind).Is_reachable()
    //             do_flag=false;
    //             break;
    //           }
    //         }
    ...
    //   if(do_flag)
    //   {
    //     Eigen::Vector3d pos3d_1;
    //     Eigen::Vector3d pos3d_2;
    //     if(uav_viewpoint_manager->InRange_Hier_ind(uav_viewpoint_manager->Robot_Hier_ind_) && 
    //     uav_viewpoint_manager->InRange_Hier_ind(A_selected_viewpoint_Hier_ind))
    //     {
    //       auto point_1 = uav_viewpoint_manager->Get_littlebox_from_Hierarchy_ind(uav_viewpoint_manager->Robot_Hier_ind_)->GetPosition();
    //       auto point_2 = uav_viewpoint_manager->Get_littlebox_from_Hierarchy_ind(A_selected_viewpoint_Hier_ind)->GetPosition();
    //       pos3d_1.x() = point_1.x;
    //       pos3d_1.y() = point_1.y;
    //       pos3d_1.z() = point_1.z;
    //       pos3d_2.x() = point_2.x;
    ...
    // {
    //   std::cout<<"-------------"<<std::endl;
    //   std::cout<<"selected_viewpoint_count "<<selected_viewpoint_count <<std::endl;
    //   std::cout<<"candidate_count "<<candidate_count <<std::endl;
    //   std::cout<<"above_frontier_threshold_count "<<above_frontier_threshold_count <<std::endl;
    //   std::cout<<"above_small_threshold_count "<<above_small_threshold_count <<std::endl;
    //   std::cout<<"above_big_threshold_count "<<above_big_threshold_count <<std::endl;
    //   std::cout<<"above_small_threshold_count "<<above_small_threshold_count <<std::endl;
    //   std::cout<<"above_small_threshold_count "<<above_small_threshold_count <<std::endl;
    // }
  ...
  global_cell_covered_marker_vec = global_cell_covered_marker_vec_;
  global_cell_covered_marker_vec_.clear();
  // for (const auto& cell_ind : almost_covered_cell_indices_)//进入almost_covered_cell_indices_的那些方块，如果随着移动 淡出局部路径规划 则自动认为已经cover
  // {
...
exploration_path_ns::ExplorationPath GridWorld::SolveGlobalTSP(
    const std::shared_ptr<uav_viewpoint_manager_ns::ViewPointManager>& uav_viewpoint_manager, std::vector<int>& global_cell_covered_marker_vec,
    std::vector<int>& ordered_cell_indices, const std::unique_ptr<keypose_graph_ns::KeyposeGraph>& keypose_graph)
{
  ...
  global_path_robot_position = robot_position_;
  ...
  else
  {
    ...
    for (int i = 0; i < neighbor_cell_indices_.size(); i++)
    {
      ...
      if (subspaces_->GetCell(cell_ind).IsRoadmapConnectionPointSet())
      {
        ...
        if (uav_viewpoint_manager->InLocalPlanningHorizon(roadmap_connection_point))
        {
          ...
          (subspaces_->GetCell(i).GetViewPointIndices().empty() && subspaces_->GetCell(i).GetVisitCount() > 1))
      {
        ...
        else
        {
          ...
          if (reachable)
          {
            ...
            exploring_cell_indices.push_back(i);
            subspaces_->GetCell(i).Setconnection_point_geo(connection_point_geo);
          }
          subspaces_->GetCell(i).Setreachable(reachable);
        }
      }
    }
  }
  ...
  global_viewpoint_cloud->cloud_->points.clear();
  pcl::PointXYZI point;
  for(auto node : global_path.nodes_)
  {
    ...
    global_viewpoint_cloud->cloud_->points.push_back(point);//可视化
  }


      
  global_viewpoint_cloud->Publish();
  return global_path;
}
...
void GridWorld::AddPathsInBetweenCells(const std::shared_ptr<uav_viewpoint_manager_ns::ViewPointManager>& uav_viewpoint_manager,
                                       const std::unique_ptr<keypose_graph_ns::KeyposeGraph>& keypose_graph)
{
  ...
  for (int i = 0; i < neighbor_cell_indices_.size(); i++)
  {
    ...
    if (subspaces_->GetCell(cell_ind).IsRoadmapConnectionPointSet())
    {
      if (uav_viewpoint_manager->InLocalPlanningHorizon(subspaces_->GetCell(cell_ind).GetRoadmapConnectionPoint()) &&
          !uav_viewpoint_manager->InCollision(subspaces_->GetCell(cell_ind).GetRoadmapConnectionPoint()))
      {
        continue;
      }
      else
    ...
    std::vector<int> candidate_viewpoint_Hier_indices = subspaces_->GetCell(cell_ind).GetViewPointIndices();
    if (!candidate_viewpoint_Hier_indices.empty())
    {
      double min_dist = DBL_MAX;
      double min_dist_viewpoint_Hier_ind = candidate_viewpoint_Hier_indices.front();
      for (const auto& viewpoint_Hier_ind : candidate_viewpoint_Hier_indices)
      {
        ...
        if (dist_to_cell_center < min_dist)
        {
          ...
          min_dist_viewpoint_Hier_ind = viewpoint_Hier_ind;
        }
      }
      geometry_msgs::msg::Point min_dist_viewpoint_position =
          uav_viewpoint_manager->Get_littlebox_from_Hierarchy_ind(min_dist_viewpoint_Hier_ind)->GetPosition();
  ...
  for (int i = 0; i < neighbor_cell_indices_.size(); i++)
  {
    ...
    if (!uav_viewpoint_manager->InLocalPlanningHorizon(from_cell_roadmap_connection_position))
    {
      continue;
    }
    ...
    for (int j = 0; j < nearby_cell_indices.size(); j++)
    {
      ...
      if (!AreNeighbors(from_cell_ind, to_cell_ind))
      {
        ...
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("my_logger"), "Cell " << from_cell_ind << " and " << to_cell_ind << " are not neighbors");
      ...
      if (!uav_viewpoint_manager->InLocalPlanningHorizon(to_cell_roadmap_connection_position))
      {
        continue;
      }
      ...
      nav_msgs::msg::Path path_in_between = uav_viewpoint_manager->GetViewPointShortestPath(
      ...
      if (PathValid(path_in_between, from_cell_ind, to_cell_ind))
      {
        ...
        keypose_graph->AddPath(path_in_between);
        // uav_keypose_graph->AddPath(path_in_between);
