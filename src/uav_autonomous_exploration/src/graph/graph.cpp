...
void Graph::SetNodePosition(int node_index, const Eigen::Vector3d& position)
{
  ...
  else
  {
    ...
    //                                                         << positions_.size() - 1 << "]");
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("my_logger"), "Graph::SetNodePosition: node_index: " << node_index << " not in range [0, "
                                                            << positions_.size() - 1 << "]");
...
void Graph::AddOneWayEdge(int from_node_index, int to_node_index, double distance)
{
  ...
  else
  {
    ...
    //                                                            << "]");
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("my_logger"), "Graph::AddOneWayEdge: from_node_index: " << from_node_index << " to_node_index: " << to_node_index
                                                               << " not in range [0, " << connection_.size() - 1
                                                               << "]");
