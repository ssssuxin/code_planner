...
bool ViewPointManager::UpdateRobotPosition(const Eigen::Vector3d& robot_position)
{
  ...
  // std::cout<<"------------"<<std::endl;
  // std::cout<<diff(0) / (vp_.kRolloverStepsize(0) * vp_.kResolution(0))<<"  "<<static_cast<int>(diff(0) / (vp_.kRolloverStepsize(0) * vp_.kResolution(0)))<<std::endl;
  // std::cout<<diff(1) / (vp_.kRolloverStepsize(1) * vp_.kResolution(1))<<"  "<<static_cast<int>(diff(1) / (vp_.kRolloverStepsize(1) * vp_.kResolution(1)))<<std::endl;
...
void ViewPointManager::GetVisualizationCloud(pcl::PointCloud<pcl::PointXYZI>::Ptr& vis_cloud, pcl::PointCloud<pcl::PointXYZI>::Ptr& visited_cloud)
{
  vis_cloud->clear();
  visited_cloud->clear();
  for (int i = 0; i < vp_.kViewPointNumber; i++)
  {
    if (IsViewPointCandidate(i, true))
    {
      ...
      {
        visited_cloud->points.push_back(vis_point);
  ...
  if(visited_cloud->points.size()<=10)
  {
    std::cout<<"有问题，visited点只有： "<<visited_cloud->points.size()<<std::endl;
  }
}
